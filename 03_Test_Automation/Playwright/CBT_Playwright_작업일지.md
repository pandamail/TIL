...
### user-exam.spec.ts 테스트 디버깅 및 안정화 (2025-10-31)

   - 최종 목표: step6의 사용자 시험 응시 세부 기능 테스트(user-exam.spec.ts)가 모두 통과하도록 안정화하는 것.

   - 과정:

       1. '메모 기능' 테스트 실패로 디버깅 시작:
           - 최초 원인을 '메모' 버튼 클릭 후 나타나는 textarea를 찾지 못하는 문제로 파악하고 수정을
             시도했습니다.

       2. '오답 표시' 기능의 예상치 못한 실패 발생 (Side Effect):
           - '메모 기능' 수정 과정에서, 이전에 잘 동작하던 '오답 표시' 기능이 갑자기 실패하는 문제가
             발생했습니다.
           - 원인 분석 (시행착오):
               - error_log.txt와 오답버튼 예시.png를 분석하며, 보기 텍스트에 취소선(line-through)이 적용되는
                 요소를 찾는 로케이터가 잘못되었음을 발견했습니다.
               - <pre>, <div>, <div> > <span> 등 여러 선택자를 시도했으나 모두 실패했습니다.
           - 핵심 해결: 최종적으로, 취소선 스타일이 특정 내부 태그가 아닌 보기 항목 전체를 감싸는 `<li>` 태그 
             자체에 적용된다는 가설을 세우고, 검증 대상을 <li>로 변경하여 '오답 표시' 기능을 성공적으로
             수정했습니다.

       3. '메모 기능' 테스트로 복귀 및 심층 디버깅:
           - '오답 표시' 기능이 해결된 후, 다시 '메모 기능' 테스트에 집중했습니다.
           - 사용자님의 결정적인 제보:
               - (제보 1) '메모' 버튼 클릭 시 생성되는 정확한 HTML 구조를 제공해주셔서, 메모장이
                 div.react-draggable 컨테이너로 생성됨을 명확히 인지했습니다.
               - (제보 2) 다른 문제로 이동하는 로직(getByRole('button', { name: '2' }))이 '답안 표기란'의 문제
                 번호가 아닌, 문제의 '보기 2번'을 클릭할 수 있는 모호한 로케이터임을 지적해주셨습니다.
           - 적용된 수정: 지적에 따라, 문제 이동 로직을 '답안 표기란' 내부에서만 버튼을 찾도록 수정하여
             안정성을 높였습니다.

       4. 최종 원인 'Stale Element' 문제 발견:
           - 모든 수정 후에도 테스트가 실패하는 마지막 원인을 분석했습니다.
           - 다른 문제로 이동했다가 다시 돌아와서 메모장을 열 때, 테스트 코드가 이전에 사라졌던 '오래된(Stale)'
              메모장 요소를 참조하려고 시도하여 오류가 발생하는 것을 확인했습니다.
           - 이 'Stale Element' 문제를 해결하기 위해, 메모장을 다시 연 시점에서 요소를 새로 찾는 코드를 방금
             제안드렸으나, 오늘은 여기까지 진행하고 정리하기로 했습니다.

  요약: 오늘은 '오답 표시' 기능의 숨은 버그를 잡고, '메모 기능'의 여러 문제점을 분석하여 최종 원인인 'Stale
  Element' 문제까지 도달하는 성과가 있었습니다.


### 메모 기능 테스트, 필터링 테스트 만들기 시작(2025-11-03)

오늘의 주요 진행 상황:

   * '메모 기능' 테스트 완성: 'Stale Element' 문제를 완전히 해결하여 '메모 기능' 테스트를 최종적으로 안정화했습니다.
   * '필터링 기능' 테스트 시작:
       * '전체 문제' 필터가 올바르게 동작하는지 '답안 표기란'의 <li> 개수를 통해 확인하는 테스트를 구현하고 통과시켰습니다.
       * '체크 문제' 필터가 버튼의 카운트 (숫자)를 올바르게 반영하는지 확인하는 테스트를 구현하고 통과시켰습니다.

  내일 할 일:

   * 오늘 구현한 '체크 문제' 테스트 로직을 다시 검토하고 개선합니다.
   * '미응답 문제' 필터의 동작 방식을 파악하고 테스트를 구현합니다.

### user-exam.spec.ts 테스트 디버깅 및 안정화 (2025-11-04)

   - 최종 목표: step6의 사용자 시험 응시 세부 기능 테스트(user-exam.spec.ts)가 모두 통과하도록 안정화하는 것.

   - 과정:
       1. '체크 문제' 필터링 테스트 로직 수정:
           - '체크 문제' 필터링 테스트에서 발생한 Strict Mode 위반, 클릭 방해, 로케이터 모호성 문제를 해결하기 위해 로케이터를 여러 차례 수정했습니다.
           - 필터 버튼 로케이터를 `userPage.getByTitle('체크 문제').filter({ hasNot: userPage.locator('div.question-container'), hasText: '(' });` 로 변경했습니다.
           - 필터링된 문제 개수 검증 로케이터를 `li:has(svg)`로 변경하여 타이밍 문제를 해결했습니다.
           - 사용자님의 제안에 따라 테스트 흐름을 1번 문제에만 체크하고 필터링 결과를 검증하는 방식으로 간결화했습니다.
           - '체크 문제' 해제 로직에서 `questionArea.locator('div.relative.border').first().getByTitle('체크 문제').click();` 로 로케이터를 수정했지만, 아직 완전히 해결되지 않은 것으로 보입니다.
